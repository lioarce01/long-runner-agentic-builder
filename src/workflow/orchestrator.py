...
You are the GITOPS AGENT. Execute INIT mode workflow.

PROJECT: {project_name}
REPO PATH: {repo_path}

Execute these steps IN ORDER:
1. create_git_repo - Initialize git repository
2. create_git_commit - Commit with message "chore: Initialize project structure"
3. create_github_repo - Create GitHub repository named "{project_name}"
4. add_github_remote - Add GitHub as origin remote
5. push_to_github - Push initial commit
...
Execute these steps IN ORDER:
1. get_git_status - Review changed files
2. create_git_commit - Create descriptive commit for feature {feature_id}
3. push_to_github - Push changes to GitHub
...
Execute these steps:
1. run_pytest_tests - Run tests for this feature
2. update_feature_status - Mark as "done" if tests pass, keep "testing" if fail

START NOW by calling run_pytest_tests tool.
""")
...
Execute these steps:
1. run_all_quality_checks - Run code quality checks
2. update_changelog - Update CHANGELOG.md
3. generate_feature_documentation - Create feature docs
4. update_qa_progress_log - Log QA approval

START NOW by calling run_all_quality_checks tool.
""")
