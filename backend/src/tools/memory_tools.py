...
def cleanup_tool_messages(messages: list, keep_last_n_tools: int = 2) -> list:
    ...
    Truncates kept ToolMessages to 500 chars (Phase 4.2 optimization).
    ...
    Returns:
        Cleaned messages list with truncated ToolMessages
    """
    ...
    # Keep only last N tool messages with truncated content (Phase 4.2)
    if keep_last_n_tools > 0 and tool_messages:
        kept_tools = tool_messages[-keep_last_n_tools:]

        # Truncate content to 500 chars (moderate - balances reduction & context)
        truncated_tools = []
        for tool_msg in kept_tools:
            content_str = str(tool_msg.content)
            truncated_content = content_str[:500]
            ...
            truncated_tools.append(truncated_msg)

        cleaned.extend(truncated_tools)
...
def create_feature_context_message(state: dict) -> str:
    """
    Create a minimal context summary for the current feature.
    Reduced from 200-300 tokens to ~50-75 tokens.

    Args:
        state: Current workflow state

    Returns:
        Compact context string (~50-75 tokens)
    """
    import glob
    ...
    # Only add test details if testing/failed (most critical info)
    if status in ["testing", "failed"]:
        try:
            ...
            if os.path.exists(test_results_dir):
                ...
                if test_files:
                    with open(test_files[0], "r", encoding="utf-8") as f:
                        ...
                        return f"{feature_id} ({status}): Tests {passed}/{total}"
        except Exception:
            pass

    # Default: just ID and status
    return f"{feature_id} ({status})"
